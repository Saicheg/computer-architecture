...
__device__ Matrix GetSubMatrix(Matrix A, int row, int col) {
	...
	Asub.elements = &A.elements[A.stride * CUDA_BLOCK_SIZE * row + CUDA_BLOCK_SIZE * col];
	return Asub;
}
...
void CudaMatrixMultiplication(const struct Matrix A, const struct Matrix B, struct Matrix C) {
	struct Matrix d_A;
	d_A.width = d_A.stride = A.width;
	d_A.height = A.height;
	size_t size = A.width * A.height * sizeof(float);
	CUDA_CHECK_RETURN(cudaMalloc(&d_A.elements, size))
	CUDA_CHECK_RETURN(cudaMemcpy(d_A.elements, A.elements, size, cudaMemcpyHostToDevice))
	...
	CUDA_CHECK_RETURN(cudaMemcpy(d_B.elements, B.elements, size, cudaMemcpyHostToDevice));
	...
	CUDA_CHECK_RETURN(cudaMemcpy(C.elements, d_C.elements, size, cudaMemcpyDeviceToHost))
...
void CpuMatrixMultiplication(const struct Matrix A, const struct Matrix B, struct Matrix C) {
...
int main(int argc, char* argv[]) {
	...
	C_cuda.elements = (float*) malloc(C_cuda.width * C_cuda.height * sizeof(float));
	...
	unsigned long long time_start_cpu, time_stop_cpu, time_start_cuda, time_stop_cuda;
    ...
    for (long i = 0; i < C_cpu.width*C_cpu.height; i++) {
        if (fabs((float)(1 - C_cuda.elements[i] / C_cpu.elements[i])) > (MATRIX_MAX_ERROR_PERCENT / 100.0)) {
            if (error_count < 10)
                printf("Got %f while expected %f\n", C_cuda.elements[i], C_cpu.elements[i]);
            error_count++;
        }
    }

    unsigned long long time_cpu  = time_stop_cpu - time_start_cpu,
                       time_cuda = time_stop_cuda - time_start_cuda;
